...
class RNASSDataGeneratorPartition(data.Dataset):
    def __init__(self, data_file, upsampling=False):
    ...
    def load_data(self):
        ...
        # self.data_x = np.array([instance[0] for instance in self.data])  # 1305 600 3
        # self.data_y = np.array([instance[1] for instance in self.data])  # 1305 600 3
        ...
        #pdb.set_trace()
        self.data_x = np.zeros((len(self.data), 600, 4))
        for i, instance in enumerate(self.data):
            onehot_seq = np.array(instance['onehot_seq'])
            seq_len = onehot_seq.shape[0]
            self.data_x[i, :seq_len, :] = onehot_seq
        self.seq_length = np.array([len(instance['seq']) for instance in self.data])
        self.len = len(self.data)
        self.seq = list([instance['seq'] for instance in self.data])
        self.seq_max_len = max([len(instance['seq']) for instance in self.data])
        self.data_name = np.array([instance['name'] for instance in self.data])
	...
	#data_len = np.nonzero(self.data_x[index].sum(axis=2))[0].max()
        ...
        matrix_rep = np.zeros((data_len,data_len))
        return  data_seq,seq_raw, matrix_rep, data_len, data_name



    def __getitem__(self, index):
        if data_name == 'NC_039208.1_1#107'
        data_seq, seq_raw,matrix_rep, data_len, data_name = self.get_one_sample(index)
        #contact, data_seq, matrix_rep, data_len, data_name, data_pair = self.data.get_one_sample_addpairs(index)
        l = get_cut_len(data_len,80)   # 最小为80长度
        ...
        data_nc = data_nc.sum(axis=0).astype(np.bool_) # 表示位置 (x, y) 是否存在至少一种核苷酸配对。
        ...
        return  data_fcn_2,seq_raw, matrix_rep, data_len, data_seq[:l], data_name, data_nc,l
...
class RNASSDataGenerator(object):
    ...
    def load_data(self):
        ...
        if self.upsampling:
            self.data = self.upsampling_data_new()
        self.data_x = np.array([instance[0] for instance in self.data])  # one hot 编码 5401 600 4
        self.data_y = np.array([instance[1] for instance in self.data])  # ss的one hot 编码 5401 600 3
        # self.pairs = np.array([instance[-1] for instance in self.data])
        self.pairs = [instance[-1] for instance in self.data]  # 5401
        #pdb.set_trace()
        self.seq_length = np.array([instance[2] for instance in self.data])
        self.len = len(self.data)
        self.seq = list(p.map(encoding2seq, self.data_x))   # 转成序列
...
class RNASSDataGenerator_input(object):
    ...
    def load_data(self):
        ...
        self.data_x = np.array([self.one_hot_3000(item) for item in self.seq])
        self.seq_max_len = 3000
    ...
    def one_hot_600(self,seq_item):
        ...
        if len(seq_item) <= 600:
            ...
            seq_item = seq_item[:600]
            feat = feat[:600]
            one_hot_matrix_600 = np.zeros((600,4))  # orig
    ...
    def one_hot_3000(self,seq_item):
        RNN_seq = seq_item
        BASES = 'AUCG'
        bases = np.array([base for base in BASES])
        feat = np.concatenate(
                [[(bases == base.upper()).astype(int)] if str(base).upper() in BASES else np.array([[-1] * len(BASES)]) for base
                in RNN_seq])
        if len(seq_item) <= 3000:
            one_hot_matrix_3000 = np.zeros((3000,4))
        else:
            seq_item = seq_item[:3000]
            feat = feat[:3000]
            one_hot_matrix_3000 = np.zeros((3000,4))  # orig
            # one_hot_matrix_3000 = np.zeros((len(seq_item),4))
        one_hot_matrix_3000[:len(seq_item),] = feat
        return one_hot_matrix_3000
    def get_one_sample(self, index):
