...
##      add fine tune
        ...
        (tp,tn,fp,fn,accuracy, prec, recall, sens, spec, F1, MCC   ) =  rna_evaluation_modified(map_no_train.cpu()[0],contacts_batch.cpu()[0],seq_name[0])
        res_list.append({
        'id': seq_name[0],
        'len': seq_name[0],
        'TP' : tp,
    ...
    result_df.to_csv(os.path.join('./results', f'{config.data.dataset}_result_xz_evaluation.csv'), index=False, header=True)
