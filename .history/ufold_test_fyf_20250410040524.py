...
##      add fine tune
        ...
        #pdb.set_trace()
        assert len(map_no_train)
        result_no_train_tmp = list(map(lambda i: rna_evaluation_modified(map_no_train.cpu()[i],
            contacts_batch.cpu()[i],l_len[i]), range(contacts_batch.shape[0])))
        result_no_train += result_no_train_tmp
        
        if nc_map_nc.sum() != 0:  # 是否存在碱基配对  这个似乎不要用，F1 和 recall非常低，为0 precision基本为1
            #pdb.set_trace()
