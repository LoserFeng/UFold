...
from .data_generator_xz import Dataset, diff_collate_fn
from .data_generator_xz_partition import Dataset as Dataset_partition, diff_collate_fn as diff_collate_fn_partition
from os.path import join
from functools import partial

dataset_choices = ['RNAStrAlign', 'archiveII', 'bpRNA', 'bpRNAnew', 'pdbnew', 'all','xz']

# ROOT_PATH = '/local4/local_dataset/RNADiffFold_data/preprocessed'
ROOT_PATH = '/local4/local_dataset/RNA_BenchMark/SS/data/xz/fyf_for_RNA_Diff'
...
def get_data(args, alphabet):
    ...
    elif args.dataset == 'xz':
        train = Dataset([join(ROOT_PATH, 'train')],name='train', upsampling=True)
        val = Dataset([join(ROOT_PATH, 'valid')], name='valid')
        test = Dataset([join(ROOT_PATH, 'test')], name='test')
    else:
