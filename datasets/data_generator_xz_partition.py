...
class ParserData(object):
    ...
    def preprocess_data(self):
        ...
        for item in self.data:
            # data_seq = item['seq_raw']
            data_length = item['length']
            # data_fcn_2 = get_data_fcn(data_seq, data_length, self.set_max_len)
            data_fcn_2 = item['data_fcn_2']
            # contact = self.pairs2map(item['pairs'], self.set_max_len)
        ...
        return None, data_fcn_2_array, data_seq_raw_list, data_length_list, data_name_list, self.set_max_len, data_seq_encode_pad_array



class Dataset(data.Dataset):

    def _generate_cache_signature(self):
        """生成基于文件元数据和配置参数的唯一签名"""
        ...
        for path in self.samples:
            st = os.stat(path)
            signature.update(f"{path}:{st.st_mtime}:{st.st_size}".encode())
            
        # 2. 配置参数签名
        config_params = {
            'upsampling': self.upsampling,
            'augment_factor': 3,  # 硬编码的增强系数
            'max_len_policy': [160, 320, 640]  # 长度筛选策略
        }
        signature.update(repr(config_params).encode())
    ...
    def upsampling_data(self):
        RNA_SS_data = collections.namedtuple('RNA_SS_data', 'contact data_fcn_2 seq_raw length name')
        final_data_file_path = os.path.join('./cache',f'{self.name}_final_data_list.npz')
        if os.path.exists(final_data_file_path):
            final_data_list = np.load(final_data_file_path,allow_pickle=True)['final_data_list'].tolist()
            print(f"Loaded final_data_list cache: {final_data_file_path}")
            ...
            np.savez_compressed(
                final_data_file_path,
                final_data_list=np.array(final_data_list, dtype=object),
                params_hash='LoserFeng'
            )
            print(f"Saved final_data_list cache: {final_data_file_path}")  
        return final_data_list
    ...
    def __getitem__(self, index: int):
        ...
        _, data_fcn_2_array, data_seq_raw_list, data_length_list, data_name_list, set_max_len, \
        ...
        return data_fcn_2, data_seq_raw_list, data_length, data_name_list, set_max_len, data_seq_encode_pad
...
def diff_collate_fn(batch, alphabet):
    data_fcn_2, data_seq_raw_list, data_length, data_name_list, set_max_len, data_seq_encode_pad = zip(*batch)
    if len(data_seq_raw_list) == 1:
        # contact = contact[0]
    ...
    return data_seq_raw,data_fcn_2, tokens, data_length, data_name, set_max_len, data_seq_encode_pad
