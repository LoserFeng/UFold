...
def structure_combine(predict_sub_bpseq_root_path,truth_bpseq_root_path ,predict_bpseq_root_path):  #合并多个片段为完整的  path_i为包含多个文件的seq1#100.bpseq ，   合并的后的文件存放的文件夹
    ...
    files_exp = [file for file in os.listdir(predict_sub_bpseq_root_path) if file.endswith('.bpseq') and '#' in file]
...
def eval_result(complete_predict_root_path,truth_ss_root_path,result_save_path,pretrained_weight_name):
    ...
    result_df.to_csv(os.path.join(result_save_path, f'{pretrained_weight_name}_bpRNA_new_partition_result.csv'), index=False, header=True)
...
def main():
    ...
    pretrained_weight_name=MODEL_SAVED.split('/')[-1].split('.')[0]
    result_save_path=f'/local4/fyf/TEST/UFold/results/partition_predict_result/{pretrained_weight_name}'
    truth_ss_root_path='/local4/local_dataset/RNA_BenchMark/SS/data/bpRNAnew/bpseq_gt'
    sub_predict_result_path=os.path.join(result_save_path,'sub_predict')
    complete_predict_root_path=os.path.join(result_save_path,'complete_predict')
    os.makedirs(result_save_path, exist_ok=True)
    os.makedirs(sub_predict_result_path, exist_ok=True)
